...
class MemoryControllerRandomFixedWindow(AbstractMemoryController):
    """
    ...
    def __init__(self, configuration, logger):
        ...
        """
        super(MemoryControllerRandomFixedWindow, self).__init__(configuration, logger)
    ...
    def update_memory(self, observation, action, reward, next_observation, done):
        ...
        if self.in_memory_capacity > 0:
            if np.random.rand() <= 0.5: # randomly decide to either push the event to the memory or not
